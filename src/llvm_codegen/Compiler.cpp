...
namespace phantom {
  namespace llvm_codegen {
    bool Compiler::compile() {
      ...
      auto module = std::make_shared<llvm::Module>(opts.output_file, *context);
      ...
      if (!generate_host_infos(module, error))
        logger.log(Logger::Level::WARNING, "Cannot get HOST informations: " + error, true);

      Visitor visitor(context, builder, module, this->logger);
      ...
      if (err_code) {
        logger.log(Logger::Level::FATAL, "Error opening file: " + err_code.message(), true);
        return false;
      }

      if (strcmp(opts.out_type.c_str(), "llvm-ir") == 0)
      ...
      if (strcmp(opts.out_type.c_str(), "obj") == 0)
        ...
        return emit_exec(file, opts.output_file, module);
    ...
    bool Compiler::emit_asm(llvm::raw_fd_ostream& file, std::shared_ptr<llvm::Module> module) {
      ...
      if (target_machine->addPassesToEmitFile(pass, file, nullptr, filetype)) {
        file.close();
        logger.log(Logger::Level::FATAL, "target_machine can't emit assembly", true);
      }
    ...
    bool Compiler::emit_obj(llvm::raw_fd_ostream& file, std::shared_ptr<llvm::Module> module) {
      ...
      if (target_machine->addPassesToEmitFile(pass, file, nullptr, filetype)) {
        file.close();
        logger.log(Logger::Level::FATAL, "target_machine can't emit an object file", true);
      }
    ...
    bool Compiler::emit_exec(llvm::raw_fd_ostream& file, const std::string& file_path,
      ...
      if (error_code) {
        file.close();
        logger.log(Logger::Level::FATAL, "Cannot create temporary file: " + error_code.message(), true);
      }

      llvm::raw_fd_ostream obj_file(temp_obj_path, error_code, llvm::sys::fs::OF_None);
      if (error_code) {
        file.close();
        logger.log(Logger::Level::FATAL, "Cannot open temp file: " + error_code.message(), true);
      }
      ...
      if (!clang_path) {
        ...
        file.close();
        logger.log(Logger::Level::FATAL, "Could not find 'clang' executable", true);
      }
      ...
      if (result != 0) {
        file.close();
        logger.log(Logger::Level::FATAL, "Linking failed: " + error_msg, true);
      }
    ...
    void Compiler::remove_file(const std::string& path) {
      ...
      if (error_code)
        logger.log(Logger::Level::FATAL, "Failed to remove file " + path + ": " + error_code.message(), true);
    }
  } // namespace llvm_codegen
} // namespace phantom
